...
export default function Hero() {
  return (
    <section className="relative flex min-h-[620px] flex-col items-center overflow-hidden bg-[#FDFCFA] px-5 sm:px-8 lg:flex-row lg:px-20">
      ...
      <div className="relative z-10 w-full py-12 lg:flex-[0_0_52%] lg:py-20">
        ...
        <h1 className="font-display mb-5 text-[clamp(2.75rem,11vw,68px)] leading-[1.0] font-black tracking-[-0.03em] text-[#111] lg:text-[68px]">
          We Clean.
          <br />
          <span className="text-redro-red italic">You Relax.</span>
        </h1>

        <p className="mb-9 max-w-[480px] text-base leading-[1.75] text-[#555] sm:text-lg">
          Professional end of lease, move-in &amp; move-out cleaning across
          Sydney — plus carpet, driveway &amp; balcony add-ons, backed by our
          100% bond back guarantee.
        </p>

        <div className="mb-13 flex flex-col items-stretch gap-3.5 sm:flex-row sm:items-center">
          <a
            href="#contact"
            className="font-display group inline-flex items-center justify-center gap-2.5 rounded-[5px] bg-redro-red px-9 py-4.5 text-sm font-bold tracking-[0.05em] text-white uppercase shadow-[0_6px_20px_rgba(212,31,31,0.3)] transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#b81b1b] hover:shadow-[0_10px_28px_rgba(212,31,31,0.4)]"
          >
            <svg
              width="16"
              height="16"
          ...
          <a
            href="#contact"
            className="font-display group inline-flex items-center justify-center gap-2.5 rounded-[5px] border-2 border-[#222] bg-white px-7 py-4 text-sm font-bold tracking-[0.05em] text-[#222] uppercase transition-all duration-200 hover:-translate-y-0.5 hover:border-redro-red hover:bg-redro-red hover:text-white hover:shadow-[0_10px_24px_rgba(212,31,31,0.25)]"
          >
            <svg
              width="16"
              height="16"
        ...
        <div className="grid grid-cols-2 overflow-hidden rounded-[10px] border border-redro-border bg-white sm:grid-cols-4">
          {STATS.map((stat, i) => (
            <div
              key={stat.label}
              className={`border-redro-border px-4 py-5 text-center sm:px-7 ${
                i % 2 === 1 ? "border-l" : ""
              } ${i >= 2 ? "border-t sm:border-t-0" : ""} ${i > 0 ? "sm:border-l" : ""}`}
            >
              <div
                className={`font-display mb-1.5 text-[28px] font-black sm:text-[32px] ${
