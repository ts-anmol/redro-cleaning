...
import { serviceLabel, bedroomLabel } from "@/types/admin";
...
const TABS: { key: string; label: string; status: LeadStatus | "ALL" }[] = [
...
export default function LeadsTable({ leads }: { leads: Lead[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter((lead) => {
      ...
      if (!q) return true;
      const haystack =
        `${lead.firstName} ${lead.lastName} ${lead.email}`.toLowerCase();
  ...
  const columns = useMemo<ColumnDef<Lead>[]>(
    () => [
      ...
      { header: "Preferred Date", accessorKey: "preferredDate" },
      {
        ...
        cell: (info) => bedroomLabel(info.getValue<string>()),
      },
      {
        header: "Status",
        accessorKey: "status",
        cell: (info) => <StatusBadge status={info.getValue<LeadStatus>()} />,
      },
      {
        header: "Received",
      ...
      },
      {
        ...
        cell: (info) => (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              router.push(`/admin/leads/${info.row.original.id}`);
            }}
